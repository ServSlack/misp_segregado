...
# Define INSTALLPATCH INSTALLATION
INSTALL_PATH=$(pwd)
...
DBHOST='MISP_DB'
DBUSER_ADMIN='root'
DBPASSWORD_ADMIN="$(random_string)" # Default on Ubuntu is a passwordless root account, if you have changed it, please set it here
DBNAME='misp'
DBPORT='3306'
DBUSER_MISP='misp'
DBPASSWORD_MISP='gaGE3qiFelK8954EutxfE4p7fcgKLWoIkOnK0jg8SWE='
...
- Admin Encription Database Key: ${ENCRIPTION_KEY}
...
declare -a packages=( git curl python3 python3-pip python3-virtualenv apache2 zip gcc sudo binutils openssl supervisor libfuzzy-dev libbrotli-dev librdkafka-dev libsimdjson-dev libzstd-dev net-tools vim ntpsec locales cron pv btop );
install_packages ${packages[@]}
error_check "Basic dependencies installation"

# Grant Brazilian Local Time:
cp files/localtime /etc/
sudo timedatectl set-timezone America/Sao_Paulo
#sudo dpkg-reconfigure -fnoninteractive tzdata
cp files/ntp.conf /etc/ntpsec/
...
if [ $INSTALL_SSDEEP == "y" ]; then
    ...
    sudo rm /usr/lib/libfuzzy.so.2
    sudo ln -s /lib/libfuzzy.so.2 /usr/lib/libfuzzy.so.2
...
print_status "Install MISP Modules"
declare -a packages=( git libpq5 libjpeg-dev tesseract-ocr libpoppler-cpp-dev imagemagick virtualenv libopencv-dev zbar-tools libzbar0 libzbar-dev libfuzzy-dev libcaca-dev python3.12-venv );
install_packages ${packages[@]}
bash -c "cd $INSTALL_PATH; sudo ./misp-modules-install.sh"
#sudo ./misp-modules-install.sh
error_check "Install MISP Modules"


print_status "Install Clamav Anti Vírus"
bash -c "cd $INSTALL_PATH; sudo ./misp-clamav-install.sh"
error_check "Install Clamav Anti Vírus"
...
# Set settings
  ...
  # ZeroMQ settings
  sudo -u ${APACHE_USER} ${MISP_PATH}/app/Console/cake Admin setSetting "Plugin.ZeroMQ_enable" true &>> $logfile
  ...
  sudo -u ${APACHE_USER} ${MISP_PATH}/app/Console/cake Admin setSetting "MISP.block_old_event_alert_age" false &>> $logfile
  sudo -u ${APACHE_USER} ${MISP_PATH}/app/Console/cake Admin setSetting "MISP.block_old_event_alert_by_date" false &>> $logfile
  ...
  sudo -u ${APACHE_USER} ${MISP_PATH}/app/Console/cake Admin setSetting "Security.alert_on_suspicious_logins" true
  ...
  sudo -u ${APACHE_USER} ${MISP_PATH}/app/Console/cake Admin setSetting "Security.encryption_key" "$ENCRIPTION_KEY"
  ...
  # Habilitar verificação de vírus com ClamAV:
  sudo -Hu ${APACHE_USER} ${MISP_PATH}/app/Console/cake Admin setSetting "Plugin.Enrichment_clamav_enabled" "true"
  sudo -Hu ${APACHE_USER} ${MISP_PATH}/app/Console/cake Admin setSetting "Plugin.Enrichment_clamav_connection" "unix:///var/run/clamav/clamd.ctl"
  sudo -Hu ${APACHE_USER} ${MISP_PATH}/app/Console/cake Admin setSetting "MISP.attachment_scan_module" "clamav"

  # Ajusta "MISP.baseurl"
  sudo -u ${APACHE_USER} ${MISP_PATH}/app/Console/cake admin setSetting "MISP.baseurl" ""

print_status "Restarting All Background Workers"
sudo supervisorctl restart all &>> $logfile
error_check "Restarting All Background Workers"
